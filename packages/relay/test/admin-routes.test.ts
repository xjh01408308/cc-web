...
async function call(method: string, url: string, opts: { session: AdminSession | null; body?: unknown; store: UserStore }): Promise<{ handled: boolean; status: number; json: unknown }> {
  const m = mockRes();
  const handled = await handleAdminUsersRoute(mockReq(method, url, opts.body), m.res, { session: opts.session, userStore: opts.store });
